            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                {revealCount >= initials.length && (
                    <div className="animate-in fade-in zoom-in duration-1000">
                        <LoadingSpinner label="Initializing Engine..." />
                    </div>
                )}
            </div>
            <div className="absolute bottom-12 text-center z-10">
                <p className="text-cyan-400/30 font-commander text-[9px] font-bold tracking-[0.5em] uppercase">Quantum Neural Architecture • 3.2</p>
            </div>
        </div>
    );
};
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[64px] shadow-[0_0_100px_rgba(0,0,0,0.8)] border-[6px] border-cyan-500/10 p-12 transform animate-in zoom-in-95 slide-in-from-bottom-10 duration-300">
            <div className="flex justify-between items-start mb-10">
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold text-cyan-500 uppercase tracking-[0.5em]">Quantum Core Selector • 3.2</p>
                </div>
                <button 
                    onClick={() => setIsOpen(false)} 
...
export const SelectionCopyPopover: React.FC<{ popover: { visible: boolean; top: number; left: number; text: string } }> = ({ popover }) => {
  ...
  const handleCopy = () => navigator.clipboard.writeText(popover.text);
  return (
    <div 
    ...
    >
      ...
      <button onClick={handleCopy} className="p-2 bg-cyan-600 rounded-xl hover:bg-cyan-500 transition-colors"><Icons.Copy className="h-4 w-4" /></button>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl border-4 border-cyan-500 p-10 transform animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black font-commander uppercase tracking-tighter text-slate-900 dark:text-white">Neural Hub</h2>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500"><Icons.X className="h-6 w-6" /></button>
        </div>
        <div className="mb-8 text-center">
          <div className="w-20 h-20 bg-cyan-600/10 rounded-[30px] flex items-center justify-center mx-auto mb-4"><Icons.Award className="h-10 w-10 text-cyan-600" /></div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium">Enter your authorization code to extend neural bandwidth.</p>
        ...
        <button onClick={() => { onRedeem(code); onClose(); }} className="w-full py-5 bg-cyan-600 text-white rounded-3xl font-black font-commander tracking-widest uppercase shadow-xl shadow-cyan-600/20 hover:bg-cyan-500 transition-all">Verify Code</button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl">
      <div className="relative w-full max-w-4xl bg-slate-900 rounded-[64px] overflow-hidden border-4 border-cyan-500/30 shadow-[0_0_150px_rgba(0,0,0,1)]">
        ...
        <div className="absolute top-8 right-8"><button onClick={onClose} className="p-5 bg-black/60 hover:bg-black/80 text-white rounded-[32px] backdrop-blur-xl border border-white/10 transition-all shadow-2xl hover:scale-110 active:scale-95"><Icons.X className="h-8 w-8" /></button></div>
        <div className="absolute bottom-16 left-0 right-0 flex flex-col items-center gap-6">
          <button onClick={capture} className="h-28 w-28 bg-white rounded-full border-[12px] border-cyan-500/40 flex items-center justify-center shadow-[0_0_60px_rgba(34,211,238,0.6)] active:scale-90 transition-all hover:scale-105"><div className="h-16 w-16 bg-cyan-600 rounded-full shadow-inner" /></button>
        </div>
      </div>
    </div>
  );
